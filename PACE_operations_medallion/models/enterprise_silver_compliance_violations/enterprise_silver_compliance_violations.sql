...
Computes lifecycle flags such as is_open.  Logic :         
- Deduplicates using latest _loaded_at per violation_id    
 - Standardises categorical fields (entity_type, severity, status)    
...
deduplicated as (
    select *,
           row_number() over (partition by violation_id order by _loaded_at desc) as _rn
    from source
...
cleaned as (
    select
        ...
        try_cast(violation_date as timestamp_ntz)                         as violation_date,
        ...
        try_cast(resolved_at as timestamp_ntz)                            as resolved_timestamp,
